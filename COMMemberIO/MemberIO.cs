...
namespace MPCOM
{
    ...
    public class MemberIO : ServicedComponent
    {
        ...
        [AutoComplete]
        public MemberData JoinMember(string account, string password, string nickname, int age, int sex, string IP, string email, string joinTime)
        {
        ...
        public MemberData MemberLogin(string Account, string Password)
        {
            ...
            try
            {
                using (SqlConnection sqlConn = new SqlConnection(this.connectionString))
                {
                    ...
                    if (DS.Tables[0].Rows.Count > 0)
                    {
                        memberData.PrimaryID = (int)Convert.ToInt16(DS.Tables[0].Rows[0]["PrimaryID"]);
                        ...
                        memberData.Age = (byte)Convert.ToInt16(DS.Tables[0].Rows[0]["Age"]);
                        memberData.Sex = (byte)Convert.ToInt16(DS.Tables[0].Rows[0]["Sex"]);
