 ...
 * ****************************************************************/
public class MatchSwitcher : MonoBehaviour
{
    #region 欄位
    ...
    void Awake()
    {
        tm = team.GetComponent<MatchManager>();
    ...
    void Update()
    {
        if (transform.childCount == 0)  // 如果沒有圖片 取消交換、拖曳功能、按鈕功能
        {
            ...
            GetComponent<MatchSwitcher>().enabled = false;
            _activeBtn = false;
    ...
    void SwitchBtn()
    {
        ...
        else if (tag == "TeamIcon" && _other.tag == "TeamIcon") // B1 > B2
        {
            if (_other.transform.childCount == 1 && _other.name != gameObject.name)  // 有物件作交換
            {
                ...
                _other.GetComponent<MatchSwitcher>()._toPos = _originPos;
    ...
    void Mice2Team(string miceID)
    {
        ...
        _other.GetComponent<MatchSwitcher>().enabled = true;
        _other.GetComponent<MatchSwitcher>().EnableBtn();

        _clone.GetComponent<MatchSwitcher>().DisableBtn();
    ...
    private void TeamSequence(GameObject teamRef, bool bDragOut)
    {
        ...
        if (bDragOut)
        {
            if (btnNo >= team.Count)
            {
                ...
                tm.infoGroupsArea[2].transform.GetChild(team.Count - offset).GetComponent<MatchSwitcher>().enabled = true;
                tm.infoGroupsArea[2].transform.GetChild(team.Count - offset).GetComponent<MatchSwitcher>().SendMessage("EnableBtn");
            }
        }
        else
        {
            if (btnNo < team.Count)                                           // 2<5
            {
                for (int i = 0; i < (team.Count - btnNo); i++)                  // 5-2=3  
                {
                    ...
                    tm.infoGroupsArea[2].transform.GetChild(btnNo + i - 1).GetChild(0).localPosition = Vector3.zero;
                    tm.infoGroupsArea[2].transform.GetChild(btnNo + i - 1).GetComponent<MatchSwitcher>().enabled = true;
                    tm.infoGroupsArea[2].transform.GetChild(btnNo + i - 1).GetComponent<MatchSwitcher>().SendMessage("EnableBtn");
                }
    ...
    private void EnDisableBtn(GameObject go, bool enable)
    {
        if (enable)
        {
            go.GetComponent<MatchSwitcher>().enabled = enable;
            go.GetComponent<MatchSwitcher>()._activeBtn = enable;
    ...
    public void DisableBtn()
    {
        ...
        GetComponent<UIDragObject>().enabled = false;
        GetComponent<BoxCollider>().isTrigger = false;
        GetComponent<MatchSwitcher>()._activeBtn = false;
        GetComponent<MatchSwitcher>().enabled = false;
        _isTrigged = false;
    }
    #endregion
    ...
    {
        GetComponent<MatchSwitcher>().enabled = true;
        GetComponent<MatchSwitcher>()._activeBtn = true;
