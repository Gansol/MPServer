...
public class Skill4 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_energy >= 0.8f && upFlag)
        {
            StartCoroutine(AnimationUp());
        }

        if (_energy < 0.8f && downFlag)
        {
    ...
    void OnHit()
    {
        if (_energy >= 0.8f)
        {
            ...
            gameManager.GetComponent<BattleManager>().UpadateEnergy(-0.8f);
        }
    }
    ...
    public IEnumerator AnimationDown() // 2   = 2 ~ 1
    {
        ...
        {
            transform.GetChild(0).localPosition = Vector3.Slerp(transform.GetChild(0).localPosition, new Vector3(0, -_upDistance * 2, 0), Time.deltaTime * 4);
        }
        yield return null;
    }
}
