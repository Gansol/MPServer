...
public class PanelManager_v2 : MonoBehaviour
{
        ...
        foreach (KeyValuePair<string, object> item in _dictObject)
        {
            ...
            if (assetBundleManager.bLoadedAssetbundle(bundleName))
            {
                ...
                if (bundle != null)
                {
                    ...
                    parent.GetChild(_objCount).GetComponent<TeamSwitcher>().enabled = true;     // 開啟老鼠隊伍交換功能
                    parent.GetChild(_objCount).GetComponent<TeamSwitcher>()._activeBtn = true;  // 開啟按鈕功能
                    parent.GetChild(_objCount).GetComponent<UIDragObject>().enabled = true;     // 開啟老鼠拖曳功能
                    _objCount++;
                }
                else
                {
