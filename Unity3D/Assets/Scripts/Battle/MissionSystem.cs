...
public class MissionSystem : IGameSystem
{
    ...
    public override void Update()
    {
        ...
        if (Global.isGameStart)
        {
            ...
            if (MissionMode == MissionMode.Completed)                               // 任務完成時，關閉任務並儲存資訊，回到初始狀態
            {
                ...
                _missionMode = MissionMode.Closed;
                _mission = Mission.None;
                AvgMissionTime = (lastGameTime + (battleSystem.GetBattleAttr().gameTime - lastGameTime)) / 2;            // 平均任務完成時間
                lastGameTime = battleSystem.GetBattleAttr().gameTime;                                                    // 任務完成時時間
            }
            ...
            if (MissionMode == MissionMode.Completing && Global.isMissionCompleted) // 任務完成中，發送完成訊息並等待Server完成資料判斷。
            {
                ...
                {
                    Global.photonService.MissionCompleted((byte)_mission, missionRate, (Int16)battleSystem.GetBattleAttr().combo, "");
                }
                else
                {
            ...
            {
                missionFlag = false;
                lastScore = battleSystem.GetBattleAttr().score;        // 儲存任務開始前的分數
    ...
    void MissionTrigger()
    {
        ...
        {
            float _otherScore = battleSystem.GetBattleAttr().otherScore;
            float _score = battleSystem.GetBattleAttr().score;
            float otherPercent = (_otherScore / (_score + _otherScore)) * 100;
            float myPercent = 100 - otherPercent;

            if ((battleSystem.GetBattleAttr().gameTime - lastGameTime) > missionInterval)                                // 任務間隔時間
            {
                ...
                if (battleSystem.GetBattleAttr().gameTime > (lastGameTime + activeTime) && seesawFlag && MissionMode == MissionMode.Closed)
                {
                ...
                if (_score > bossActiveScore && _otherScore > bossActiveScore && battleSystem.GetBattleAttr().gameTime > bossActiveTime && MissionMode == MissionMode.Closed)
                {
    ...
    void MissionExecutor(Mission mission)
    {
        switch (mission)
        {
            case Mission.Harvest:   // 達成XX收穫
                {
                    if ((battleSystem.GetBattleAttr().score - lastScore) >= _missionScore)      // success
                    {
                        _missionMode = MissionMode.Completing;
                        Global.isMissionCompleted = true;
                    }
                    else if (battleSystem.GetBattleAttr().gameTime - lastGameTime > missionTime)                            // failed
                    {
            ...
            case Mission.Reduce:        // 完成後 activeScore要減少Reduce的量
                {
                    double endTime = battleSystem.GetBattleAttr().gameTime - lastGameTime - missionTime;
                    if (endTime > -5 && endTime < 0) // 減少糧食 這比較特殊 需要顯示閃爍血調 還沒寫
                    {
                        battleUI.HPBar_Shing();
                    }
                    else if (battleSystem.GetBattleAttr().gameTime - lastGameTime > missionTime)
                    {
                        _missionMode = MissionMode.Completing;
                        Global.isMissionCompleted = true;
                    }
            ...
            case Mission.DrivingMice:
                {
                    if (battleSystem.GetBattleAttr().gameTime - lastGameTime > missionTime)       //missionScore 這裡是 Combo任務目標
                    {
            ...
            case Mission.HarvestRate:
                {
                    if (battleSystem.GetBattleAttr().gameTime - lastGameTime > missionTime)
                    {
                        _missionMode = MissionMode.Completing;
                        Global.isMissionCompleted = true;
                    }
    ...
    void OnApplyMission(Mission mission, Int16 missionScore)
    {
        if (Global.isGameStart)
        {
            ...
            _mission = mission;
            lastGameTime = battleSystem.GetBattleAttr().gameTime;                // 任務開始時時間
