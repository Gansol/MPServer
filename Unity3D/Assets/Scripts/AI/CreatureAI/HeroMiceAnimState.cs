...
public class HeroMiceAnimState : IAnimatorState
{
    ...
    public HeroMiceAnimState(GameObject obj, bool isBoss, float lerpSpeed, float upSpeed, float upDistance, float lifeTime)
    ...
    public override void UpdateAnimation()
    {
        ...
        if (anims != null)
        {
            ...
            if (currentState.nameHash == Animator.StringToHash("Layer1.Hello"))         // 如果 目前 動化狀態 是 up
            {
                ...
                {
                    if (currentState.nameHash == Animator.StringToHash("Layer1.Hello") && animState != ENUM_AnimatorState.Died) 
            ...
            else if (currentState.nameHash == Animator.StringToHash("Layer1.Eat"))
            {
                ...
                if (!_bEating)        // 限制執行一次
                {
                    if (currentState.normalizedTime > .8f && currentState.normalizedTime < 1 && !anims.IsInTransition(0))                       // 動畫撥放完畢時
                    {
                        animState = ENUM_AnimatorState.Died;  
                    }
                    else if (currentState.normalizedTime > 1 && !anims.IsInTransition(0))                       // 動畫撥放完畢時
                    {
    ...
    {
        // _tmpSpeed = Mathf.Lerp(_tmpSpeed, 1, _lerpSpeed);
        float distance = Vector3.Distance(obj.transform.position, _toWorldPos);
        if (distance >= 0 && distance <= 0.05f)
        {
            obj.transform.position = _toWorldPos;
            _toFlag = false;
        }
        else
        {
            obj.transform.position = Vector3.Lerp(obj.transform.position, _toWorldPos, _lerpSpeed);
        }
    }
    ...
    private void AnimationScale()
    {
        ...
        {
            obj.transform.localScale = Vector3.Lerp(obj.transform.localScale, _scale, _lerpSpeed);
        }
    }
