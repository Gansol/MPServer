...
public class HeroMiceAnimState : IAnimatorState
{
    ...
    {
        // _tmpSpeed = Mathf.Lerp(_tmpSpeed, 1, _lerpSpeed);
        float distance = Vector3.Distance(go.transform.position, _toWorldPos);
        if (distance >= 0 && distance <= 0.05f)
        {
            go.transform.position = _toWorldPos;
            _toFlag = false;
        }
        else
        {
            go.transform.position = Vector3.Lerp(go.transform.position, _toWorldPos, _lerpSpeed);
        }
    }
    ...
    private void AnimationScale()
    {
        ...
        {
            go.transform.localScale = Vector3.Lerp(go.transform.localScale, _scale, _lerpSpeed);
        }
    }
